module.exports = {
  TRANSLATION: {
    lang_ru: {
      questions: [
        {
          id: 1,
          text:
            "Организационные недостатки на работе постоянно заставляют нервничать, переживать, напрягаться.",
        },
        {
          id: 2,
          text:
            "Сегодня я доволен своей профессией не меньше, чем в начале карьеры.",
        },
        {
          id: 3,
          text:
            "Я ошибся в выборе профессии или профиля деятельности (занимаю не свое место).",
        },
        {
          id: 4,
          text:
            "Меня беспокоит то, что я стал хуже работать (менее продуктивно, качественнее, медленно).",
        },
        {
          id: 5,
          text:
            "Теплота взаимодействия с партнерами очень зависит от моего настроения — хорошего или плохого.",
        },
        {
          id: 6,
          text:
            "От меня как профессионала мало зависит благополучие партнеров."       
        },
        {
          id: 7,
          text:
            "Когда я прихожу с работы домой, то некоторое время (часа 2–3) мне хочется побыть наедине, чтобы со мной никто не общался.",
        },
        {
          id: 8,
          text:
            "Когда я чувствую усталость или напряжение, то стараюсь поскорее «свернуть» дело.",
        },
        {
          id: 9,
          text:
            "Мне кажется, что эмоционально я не могу дать партнерам того, что требует профессиональный долг.",
        },
        {
          id: 10,
          text:
            "Моя работа притупляет эмоции.",
        },
        {
          id: 11,
          text:
            "Я откровенно устал от проблем, с которыми приходится иметь дело на работе.",
        },
        {
          id: 12,
          text:
            "Бывает, что я плохо засыпаю (сплю) из-за переживаний, связанных с работой.",
        },
        {
          id: 13,
          text:
            "Взаимодействие с партнерами требует от меня большого напряжения.",
        },
        {
          id: 14,
          text:
            "Работа приносит мне все меньше удовлетворения.",
        },
        {
          id: 15,
          text:
            "Я бы сменил место работы, если бы представилась возможность.",
        },
        {
          id: 16,
          text:
            "Меня часто расстраивает то, что я не могу должным образом оказать партнеру профессиональную поддержку, услугу, помощь.",
        },
        {
          id: 17,
          text:
            "Мне всегда удается предотвратить влияние плохого настроения на деловые контакты.",
        },
        {
          id: 18,
          text:
            "Меня очень огорчает, если что-то не ладится в отношениях с деловым партнером.",
        },
        {
          id: 19,
          text:
            "Я настолько устаю на работе, что дома стараюсь общаться как можно меньше.",
        },
        {
          id: 20,
          text:
            "Из-за нехватки времени, усталости или напряжения часто уделяю внимание партнеру меньше, чем положено.",
        },
        {
          id: 21,
          text:
            "Иногда самые обычные ситуации общения на работе вызывают раздражение.",
        },
        {
          id: 22,
          text:
            "Я спокойно воспринимаю обоснованные претензии партнеров.",
        },
        {
          id: 23,
          text:
            "Общение с партнерами побудило меня сторониться людей.",
        },
        {
          id: 24,
          text:
            "При воспоминании о некоторых коллегах по работе или партнерах у меня портится настроение.",
        },
        {
          id: 25,
          text:
            "Конфликты или разногласия с коллегами отнимают много сил и эмоций.",
        },
        {
          id: 26,
          text:
            "Мне все труднее устанавливать или поддерживать контакты с деловыми партнерами.",
        },
        {
          id: 27,
          text:
            "Обстановка на работе мне кажется очень трудной, сложной.",
        },
        {
          id: 28,
          text:
            "У меня часто возникают тревожные ожидания, связанные с работой: что-то должно случиться, как бы не допустить ошибки, смогу ли сделать все, как надо, не сократят ли и т. п.",
        },
        {
          id: 29,
          text:
            "Если партнер мне неприятен, я стараюсь ограничить время общения с ним или меньше уделять ему внимания.",
        },
        {
          id: 30,
          text:
            "В общении на работе я придерживаюсь принципа: «не делай людям добра, не получишь зла».",
        },
        {
          id: 31,
          text:
            "Я охотно рассказываю домашним о своей работе.",
        },
        {
          id: 32,
          text:
            "Бывают дни, когда мое эмоциональное состояние плохо сказывается на результатах работы (меньше делаю, снижается качество, случаются конфликты).",
        },
        {
          id: 33,
          text:
            "Порой я чувствую, что надо проявить к партнеру эмоциональную отзывчивость, но не могу.",
        },
        {
          id: 34,
          text:
            "Я очень переживаю за свою работу.",
        },
        {
          id: 35,
          text:
            "Партнерам по работе отдаешь внимания и заботы больше, чем получаешь от них признательности.",
        },
        {
          id: 36,
          text:
            "При мысли о работе мне обычно становится не по себе: начинает колоть в области сердца, повышается давление, появляется головная боль.",
        },
        {
          id: 37,
          text:
            "У меня хорошие (вполне удовлетворительные) отношения с непосредственным руководителем.",
        },
        {
          id: 38,
          text:
            "Я часто радуюсь, видя, что моя работа приносит пользу людям.",
        },
        {
          id: 39,
          text:
            "Последнее время (или как всегда) меня преследуют неудачи в работе.",
        },
        {
          id: 40,
          text:
            "Некоторые стороны (факты) моей работы вызывают глубокое разочарование, повергают в уныние.",
        },
        {
          id: 41,
          text:
            "Бывают дни, когда контакты с партнерами складываются хуже, чем обычно.",
        },
        {
          id: 42,
          text:
            "Я разделяю деловых партнеров (субъектов деятельности) на «хороших» и «плохих».",
        },
        {
          id: 43,
          text:
            "Усталость от работы приводит к тому, что я стараюсь сократить общение с друзьями и знакомыми.",
        },
        {
          id: 44,
          text:
            "Я обычно проявляю интерес к личности партнера помимо того, что касается дела.",
        },
        {
          id: 45,
          text:
            "Обычно я прихожу на работу отдохнувшим, со свежими силами, в хорошем настроении.",
        },
        {
          id: 46,
          text:
            "Я иногда ловлю себя на том, что работаю с партнерами автоматически, без души.",
        },
        {
          id: 47,
          text:
            "По работе встречаются настолько неприятные люди, что невольно желаешь им чего-нибудь плохого.",
        },
        {
          id: 48,
          text:
            "После общения с неприятными партнерами у меня бывает ухудшение физического или психического самочувствия.",
        },
        {
          id: 49,
          text:
            "На работе я испытываю постоянные физические или психологические перегрузки.",
        },
        {
          id: 50,
          text:
            "Успехи в работе вдохновляют меня.",
        },
        {
          id: 51,
          text:
            "Ситуация, в которой я оказался на работе, кажется безысходной (почти безысходной).",
        },
        {
          id: 52,
          text:
            "Я потерял покой из-за работы.",
        },
        {
          id: 53,
          text:
            "На протяжении последнего года была жалоба (были жалобы) в мой адрес со стороны партнера(ов).",
        },
        {
          id: 54,
          text:
            "Мне удается беречь нервы благодаря тому, что многое из происходящего с партнерами я не принимаю близко к сердцу.",
        },
        {
          id: 55,
          text:
            "Я часто с работы приношу домой отрицательные эмоции.",
        },
        {
          id: 56,
          text:
            "Я часто работаю через силу.",
        },
        {
          id: 57,
          text:
            "Прежде я был более отзывчивым и внимательным к партнерам, чем теперь.",
        },
        {
          id: 58,
          text:
            "В работе с людьми я руководствуюсь принципом: не трать нервы, береги здоровье.",
        },
        {
          id: 59,
          text:
            "Иногда иду на работу с тяжелым чувством: как все надоело, никого бы не видеть и не слышать.",
        },
        {
          id: 60,
          text:
            "После напряженного рабочего дня я чувствую недомогание.",
        },
        {
          id: 61,
          text:
            "Контингент партнеров, с которым я работаю, очень трудный.",
        },
        {
          id: 62,
          text:
            "Иногда мне кажется, что результаты моей работы не стоят тех усилий, которые я затрачиваю.",
        },
        {
          id: 63,
          text:
            "Если бы мне повезло с работой, я был бы более счастлив.",
        },
        {
          id: 64,
          text:
            "Я в отчаянии из-за того, что на работе у меня серьезные проблемы.",
        },
        {
          id: 65,
          text:
            "Иногда я поступаю со своими партнерами так, как не хотел бы, чтобы поступали со мной.",
        },
        {
          id: 66,
          text:
            "Я осуждаю партнеров, которые рассчитывают на особое снисхождение, внимание.",
        },
        {
          id: 67,
          text:
            "Чаще всего после рабочего дня у меня нет сил заниматься домашними делами.",
        },
        {
          id: 68,
          text:
            "Обычно я тороплю время: скорей бы рабочий день кончился.",
        },
        {
          id: 69,
          text:
            "Состояния, просьбы, потребности партнеров обычно меня искренне волнуют.",
        },
        {
          id: 70,
          text:
            "Работая с людьми, я обычно как бы ставлю экран, защищающий от чужих страданий и отрицательных эмоций.",
        },
        {
          id: 71,
          text:
            "Работа с людьми (партнерами) очень разочаровала меня.",
        },
        {
          id: 72,
          text:
            "Чтобы восстановить силы, я часто принимаю лекарства.",
        },
        {
          id: 73,
          text:
            "Как правило, мой рабочий день проходит спокойно и легко.",
        },
        {
          id: 74,
          text:
            "Мои требования к выполняемой работе выше, чем то, чего я достигаю в силу обстоятельств.",
        },
        {
          id: 75,
          text:
            "Моя карьера сложилась удачно.",
        },
        {
          id: 76,
          text:
            "Я очень нервничаю из-за всего, что связано с работой.",
        },
        {
          id: 77,
          text:
            "Некоторых из своих постоянных партнеров я не хотел бы видеть и слышать.",
        },
        {
          id: 78,
          text:
            "Я одобряю коллег, которые полностью посвящают себя людям (партнерам), забывая о собственных интересах.",
        },
        {
          id: 79,
          text:
            "Моя усталость на работе обычно мало сказывается (никак не сказывается) в общении с домашними и друзьями.",
        },
        {
          id: 80,
          text:
            "Если предоставляется случай, я уделяю партнеру меньше внимания, но так, чтобы он этого не заметил.",
        },
        {
          id: 81,
          text:
            "Меня часто подводят нервы в общении с людьми на работе.",
        },
        {
          id: 82,
          text:
            "Ко всему (почти ко всему), что происходит на работе, я утратил интерес, живое чувство.",
        },
        {
          id: 83,
          text:
            "Работа с людьми плохо повлияла на меня как профессионала — обозлила, сделала нервным, притупила эмоции.",
        },
        {
          id: 84,
          text:
            "Работа с людьми явно подрывает мое здоровье.",
        },
      ],

      criterions: {

        TENSION_CIRCUMSTANCES: "Переживание психотравмирующих обстоятельств",
        TENSION_SELF_SATISFACTION: "Удовлетворенность собой",
        TENSION_CAGED: "«Загнанность в клетку»",
        TENSION_DEPRESSION: "Тревога и депрессия",

        RESISTANCE_EMOTIONAL_RESPONSE: "Неадекватное избирательное эмоциональное реагирование",
        RESISTANCE_DISORIENTATION: "Эмоционально-нравственная дезориентация",
        RESISTANCE_EXPANDING_THE_SCOPE: "Расширение сферы экономии эмоций",
        RESISTANCE_PROF_DUTIES: "Редукция профессиональных обязанностей",

        DEPLETION_EMOTIONAL_DEFECITS: "Эмоциональный дефицит",
        DEPLETION_EMOTIONAL_DETACHMENT: "Эмоциональная отстраненность",
        DEPLETION_DEPERSONALIZATION: "Личностная отстраненность (деперсонализация)",
        DEPLETION_PSYCHO_DISORDERS: "Психосоматические и психовегетативные нарушения",

        TENSION: "напряжение",
        RESISTANCE: "резистенция",
        DEPLETION: "истощение",

        SYMBOL_PLUS: "+",
        SYMBOL_MINUS: "-",

      },

      common: {

        WELCOME_TEXT: `Добро пожаловать в бот социальной поддержки CodeFlexers 
          \nОн поможет вам справиться с проблемами профессионального выгорания. 
          \nПосле короткого теста вы получите информацию о состоянии вашего эмоционального выгорания и рекомендации, как справиться с ситуацией. 
          \n\nНажмите /language, чтобы выбрать язык.\n\nНажмите /questionnaire чтобы начать опрос`,
        WRONG_COMMAND: "Неправильная команда, попробуйте еще раз!",
        CHOOSE_LANGUAGE_TEXT: "Какой язык вы предпочитаете?",
        LANGUAGE_CHANGED: `Язык успешно изменен. 
          \n\nНажмите /language, чтобы выбрать язык. 
          \n\nНажмите /questionnaire чтобы начать опрос`,
        ANSWER_TEXT: `1) Начинайте осваивать навыки тайм-менеджмента:
          \n\n\t-планируйте свой день: не более 6-7 дел в день;
          \n\t-правильно расставляйте приоритеты;
          \n\t-научитесь говорить «НЕТ»
          \n\n2) Приучайте Ваших клиентов к культуре делового общения.
          \nПри первой встрече с клиентом устанавливайте четкие правила вашего сотрудничества, ведь в большинстве случаев клиенты искренне убеждены, что юрист или адвокат быть на связи 24/7 - без выходных и отпусков.
          \n\n3) Физическая активность.
          \nНи для кого не секрет, что наша жизнь становится все более сидящим. Большинство рабочего времени юристы проводят за компьютером - изучая нормативно-правовую базу или готовя документы. Если бы это странно не звучало, но отсутствие физических нагрузок является одним из факторов, который может провоцировать профессиональное выгорание. Преимущества от занятий спорта очевидны, самое важное - сделать первый шаг и начать им заниматься. Вы удивитесь насколько быстро физическая активность перерастет в привычку и будет приносить Вам массу удовольствия.
          \n\n4) Не забывайте об отдыхе.
          \nВ работе всегда надо работы перерыв, не зря законодательством предусмотрена ежегодный отпуск. Какой это будет вид отдыха - поход в горы, плавание в море или просто чтение книги - выбор за Вами. Даже если Вы никуда не поедете, а просто остаться дома - это уже возможность «перезагрузиться» для организма. Только не поддавайтесь «соблазну» зайти на работу или заглянуть на рабочую почту!
          \n\n5) Здоровый сон - залог крепкого здоровья!
          \nНе стоит жертвовать сном для решения своих дел - это крайне негативно для здоровья. 7-8 часов сна - основная составляющая крепкого здоровья.
          \n\n6) Помощь специалистов.
          \nКогда возникает понимание, что самому справиться с проблемой профессионального выгорания не удается - обращайтесь за помощью к специалистам: психологу или психотерапевту.`,

      }
    },

    lang_en: {
      questions: [
        {
          id: 1,
          text:
            "Organizational flaws at work constantly make you nervous, worried, tense.",
        },
        {
          id: 2,
          text:
            "Today I am as satisfied with my profession as I was at the beginning of my career.",
        },
        {
          id: 3,
          text:
            "I made a mistake in choosing a profession or profile (I am not in my place).",
        },
        {
          id: 4,
          text:
            "My concern is that I have begun to work worse (less productive, better quality, slower).",
        },
        {
          id: 5,
          text:
            "The warmth of interaction with partners very much depends on my mood - good or bad.",
        },
        {
          id: 6,
          text:
            "As a professional, the well-being of partners hardly depends on me. "       
        },
        {
          id: 7,
          text:
            "When I come home from work, then for some time (2–3 hours) I want to be alone so that no one will talk to me.",
        },
        {
          id: 8,
          text:
            "When I feel tired or stressed, I try to \"wind down\" the case as soon as possible.",
        },
        {
          id: 9,
          text:
            "It seems to me that emotionally I cannot give my partners what a professional duty requires.",
        },
        {
          id: 10,
          text:
            "My work dulls my emotions.",
        },
        {
          id: 11,
          text:
            "I am frankly tired of the problems I have to deal with at work.",
        },
        {
          id: 12,
          text:
            "It happens that I do not fall asleep (sleep) well because of the worries associated with work.",
        },
        {
          id: 13,
          text:
            "Interaction with partners requires a lot of stress from me.",
        },
        {
          id: 14,
          text:
            "My work gives me less satisfaction.",
        },
        {
          id: 15,
          text:
            "I would change jobs if the opportunity presented itself.",
        },
        {
          id: 16,
          text:
            "I am often upset that I cannot properly provide my partner with professional support, service, assistance.",
        },
        {
          id: 17,
          text:
            "I always manage to prevent a bad mood from affecting my business contacts.",
        },
        {
          id: 18,
          text:
            "It upsets me very much if something goes wrong in a relationship with a business partner.",
        },
        {
          id: 19,
          text:
            "I get so tired at work that I try to communicate as little as possible at home.",
        },
        {
          id: 20,
          text:
            "Due to lack of time, fatigue or stress, I often pay less attention to my partner than I should.",
        },
        {
          id: 21,
          text:
            "Sometimes the most common social situations at work are annoying.",
        },
        {
          id: 22,
          text:
            "I calmly accept the well-founded claims of partners.",
        },
        {
          id: 23,
          text:
            "Communication with partners prompted me to stay away from people.",
        },
        {
          id: 24,
          text:
            "Thinking about some work colleagues or partners makes me feel bad.",
        },
        {
          id: 25,
          text:
            "Conflicts or disagreements with colleagues take a lot of energy and emotion.",
        },
        {
          id: 26,
          text:
            "I find it increasingly difficult to establish or maintain contacts with business partners.",
        },
        {
          id: 27,
          text:
            "The situation at work seems to me to be very difficult, complicated.",
        },
        {
          id: 28,
          text:
            "I often have anxious expectations related to work: something should happen, how to avoid mistakes, whether I can do everything as it should, whether they will reduce, etc.",
        },
        {
          id: 29,
          text:
            "If my partner is unpleasant to me, I try to limit the time of communication with him or pay less attention to him.",
        },
        {
          id: 30,
          text:
            "In communication at work, I adhere to the principle: \"do not do good to people, you will not get evil.\"",
        },
        {
          id: 31,
          text:
            "I gladly tell my family about my work.",
        },
        {
          id: 32,
          text:
            "There are days when my emotional state has a bad effect on the results of my work (I do less, the quality decreases, conflicts occur).",
        },
        {
          id: 33,
          text:
            "Sometimes I feel that I need to show emotional responsiveness to my partner, but I can't.",
        },
        {
          id: 34,
          text:
            "I am very worried about my work.",
        },
        {
          id: 35,
          text:
            "You give more attention and care to your work partners than you get appreciation from them.",
        },
        {
          id: 36,
          text:
            "When I think about work, I usually feel uncomfortable: begins to prick in the heart area, blood pressure rises, and a headache appears.",
        },
        {
          id: 37,
          text:
            "I have a good (quite satisfactory) relationship with my line manager.",
        },
        {
          id: 38,
          text:
            "I am often glad to see that my work is beneficial to people.",
        },
        {
          id: 39,
          text:
            "Lately (or as always) I have been haunted by failures at work.",
        },
        {
          id: 40,
          text:
            "Some aspects (facts) of my work are deeply disappointing and disheartening.",
        },
        {
          id: 41,
          text:
            "There are days when contacts with partners are worse than usual.",
        },
        {
          id: 42,
          text:
            "I classify business partners (stakeholders) as “good” and “bad”.",
        },
        {
          id: 43,
          text:
            "Fatigue from work leads to the fact that I try to reduce communication with friends and acquaintances.",
        },
        {
          id: 44,
          text:
            "I usually take an interest in my partner's personality beyond the case.",
        },
        {
          id: 45,
          text:
            "Usually I come to work refreshed, with fresh energy, in a good mood.",
        },
        {
          id: 46,
          text:
            "Sometimes I find myself working with partners automatically, without a soul.",
        },
        {
          id: 47,
          text:
            "At work, people are so unpleasant that you involuntarily wish them something bad.",
        },
        {
          id: 48,
          text:
            "After communicating with unpleasant partners, I have a deterioration in physical or mental well-being.",
        },
        {
          id: 49,
          text:
            "At work, I experience constant physical or psychological stress.",
        },
        {
          id: 50,
          text:
            "Success in my work inspires me.",
        },
        {
          id: 51,
          text:
            "The situation in which I found myself at work seems hopeless (almost hopeless).",
        },
        {
          id: 52,
          text:
            "I lost my peace of mind because of work.",
        },
        {
          id: 53,
          text:
            "Over the past year, there has been a complaint (there were complaints) addressed to me from the partner (s).",
        },
        {
          id: 54,
          text:
            "I manage to save my nerves due to the fact that I do not take much of what is happening with partners to my heart.",
        },
        {
          id: 55,
          text:
            "I often bring negative emotions home from work.",
        },
        {
          id: 56,
          text:
            "I often work hard.",
        },
        {
          id: 57,
          text:
            "Before, I was more responsive and attentive to partners than I am now.",
        },
        {
          id: 58,
          text:
            "In working with people, I am guided by the principle: do not waste your nerves, take care of your health.",
        },
        {
          id: 59,
          text:
            "Sometimes I go to work with a heavy feeling: I’m tired of everything, I wouldn’t see or hear anyone.",
        },
        {
          id: 60,
          text:
            "After a busy day at work, I feel unwell.",
        },
        {
          id: 61,
          text:
            "The contingent of partners I work with is very difficult.",
        },
        {
          id: 62,
          text:
            "Sometimes it seems to me that the results of my work are not worth the effort that I spend.",
        },
        {
          id: 63,
          text:
            "If I was lucky with my job, I would be happier.",
        },
        {
          id: 64,
          text:
            "I am desperate because I have serious problems at work.",
        },
        {
          id: 65,
          text:
            "Sometimes I act with my partners in a way that I would not want to be treated with me.",
        },
        {
          id: 66,
          text:
            "I condemn partners who count on special indulgence and attention",
        },
        {
          id: 67,
          text:
            "Most often, after a working day, I don't have the strength to do household chores.",
        },
        {
          id: 68,
          text:
            "Usually I rush the time: the working day would be over as soon as possible.",
        },
        {
          id: 69,
          text:
            "The states, requests, needs of partners usually worry me sincerely.",
        },
        {
          id: 70,
          text:
            "When working with people, I usually put on a screen that protects from other people's suffering and negative emotions.",
        },
        {
          id: 71,
          text:
            "Working with people (partners) was very disappointing to me.",
        },
        {
          id: 72,
          text:
            "To recuperate, I often take medications.",
        },
        {
          id: 73,
          text:
            "As a rule, my working day is calm and easy.",
        },
        {
          id: 74,
          text:
            "My requirements for the work performed are higher than what I achieve by force of circumstances.",
        },
        {
          id: 75,
          text:
            "My career has gone well.",
        },
        {
          id: 76,
          text:
            "I am very nervous about everything related to work.",
        },
        {
          id: 77,
          text:
            "I would not like to see and hear some of my regular partners.",
        },
        {
          id: 78,
          text:
            "I approve of colleagues who completely devote themselves to people (partners), forgetting about their own interests.",
        },
        {
          id: 79,
          text:
            "My fatigue at work usually has little effect (does not affect in any way) in communication with my family and friends.",
        },
        {
          id: 80,
          text:
            "If given the opportunity, I pay less attention to my partner, but without noticing it.",
        },
        {
          id: 81,
          text:
            "I am often let down by my nerves when dealing with people at work.",
        },
        {
          id: 82,
          text:
            "In everything (almost everything) that happens at work, I have lost interest, a living feeling.",
        },
        {
          id: 83,
          text:
            "Working with people had a bad influence on me as a professional - it made me angry, made me nervous, dulled my emotions.",
        },
        {
          id: 84,
          text:
            "Working with people clearly undermines my health.",
        },
      ],
      
      criterions: {

        TENSION_CIRCUMSTANCES: "Experiencing traumatic circumstances",
        TENSION_SELF_SATISFACTION: "Self-satisfaction",
        TENSION_CAGED: "Confined in a cage",
        TENSION_DEPRESSION: "Anxiety and depression",

        RESISTANCE_EMOTIONAL_RESPONSE: "Inappropriate selective emotional response",
        RESISTANCE_DISORIENTATION: "Emotional and moral disorientation",
        RESISTANCE_EXPANDING_THE_SCOPE: "Expanding the scope of saving emotions",
        RESISTANCE_PROF_DUTIES: "Reduction of professional duties",

        DEPLETION_EMOTIONAL_DEFECITS: "Emotional deficits",
        DEPLETION_EMOTIONAL_DETACHMENT: "Emotional detachment",
        DEPLETION_DEPERSONALIZATION: "Personal detachment (depersonalization)",
        DEPLETION_PSYCHO_DISORDERS: "Psychosomatic and psychovegetative disorders",

        TENSION: "tension",
        RESISTANCE: "resistance",
        DEPLETION: "depletion",

        SYMBOL_PLUS: "+",
        SYMBOL_MINUS: "-",

      },

      common: {

        WELCOME_TEXT: `Welcome to the CodeFlexers social support bot. 
          \nIt will help you to deal with your professional burnout problems. 
          \nAfter a short test you will receive information about your burnout state and will receive advice to handle the situation. 
          \n\nClick on /language in order to choose language. \n\nClick on /questionnaire in order to start the questionnaire.`,
        WRONG_COMMAND: "Wrong command, try again!",
        CHOOSE_LANGUAGE_TEXT: "What language do you prefer?",
        LANGUAGE_CHANGED: `Language changed successfully. 
          \n\nClick on /language in order to choose language. 
          \n\nClick on /questionnaire in order to start the questionnaire.`,
        ANSWER_TEXT: `1) Start mastering time management skills:
          \n\n\t-plan your day: no more than 6-7 tasks per day;
          \n\t-prioritize correctly;
          \n\t-learn to say NO
          \n\n2) Train your customers to a culture of business communication.
          \nWhen you first meet with a client, set clear rules for your cooperation, because in most cases clients are sincerely convinced that a lawyer or attorney can be in touch 24/7 - without days off and holidays.
          \n\n3) Physical activity.
          \nIt's no secret that our life is becoming more and more sedentary. Lawyers spend most of their working time at the computer - studying the legal framework or preparing documents. If it sounds strange, lack of physical activity is one of the factors that can provoke professional burnout. The benefits of sports are obvious, the most important thing is to take the first step and start doing it. You will be surprised how quickly physical activity becomes a habit and will bring you a lot of pleasure.
          \n\n4) Don't forget to rest.
          \nThere is always a need for a break in work, it is not for nothing that the legislation provides for annual leave. What kind of rest it will be - hiking, swimming in the sea or just reading a book - the choice is yours. Even if you don't go anywhere, but just stay at home is already an opportunity to "reboot" for the body. Just don't be tempted to go to work or check your work email!
          \n\n5) Healthy sleep is the key to good health!
          \nYou should not sacrifice sleep to solve your affairs - it is extremely negative for health. 7-8 hours of sleep is essential for good health.
          \n\n6) Help from specialists.
          \nWhen you realize that you cannot cope with the problem of professional burnout yourself, seek help from a specialist: a psychologist or a psychotherapist.`
      }
    },

    lang_ua: {
      questions: [
        {
          id: 1,
          text:
            "Організаційні недоліки на роботі постійно змушують нервувати, переживати, напружуватися.",
        },
        {
          id: 2,
          text:
            "Сьогодні я задоволений своєю професією не менше, аніж на початку кар'єри.",
        },
        {
          id: 3,
          text:
            "Я помилився у виборі професії або профілю діяльності (займаю не своє місце).",
        },
        {
          id: 4,
          text:
            "Мене турбує те, що я став гірше працювати (менш продуктивно, якісніше, повільно).",
        },
        {
          id: 5,
          text:
            "Теплота взаємодії з партнерами дуже залежить від мого настрою - хорошого або поганого.",
        },
        {
          id: 6,
          text:
            "Від мене як професіонала мало залежить благополуччя партнерів."       
        },
        {
          id: 7,
          text:
            "Коли я приходжу з роботи додому, то деякий час (години 2-3) мені хочеться побути наодинці, щоб зі мною ніхто не спілкувався.",
        },
        {
          id: 8,
          text:
            "Коли я відчуваю втому або напруга, то намагаюся скоріше «згорнути» справу.",
        },
        {
          id: 9,
          text:
            "Мені здається, що емоційно я не можу дати партнерам того, що вимагає професійний обов'язок.",
        },
        {
          id: 10,
          text:
            "Моя робота притупляє емоції.",
        },
        {
          id: 11,
          text:
            "Я відверто втомився від проблем, з якими доводиться мати справу на роботі.",
        },
        {
          id: 12,
          text:
            "Буває, що я погано засинаю (сплю) через переживання, пов'язаних з роботою.",
        },
        {
          id: 13,
          text:
            "Взаємодія з партнерами вимагає від мене великої напруги.",
        },
        {
          id: 14,
          text:
            "Робота приносить мені все менше задоволення.",
        },
        {
          id: 15,
          text:
            "Я б змінив місце роботи, якби випала нагода.",
        },
        {
          id: 16,
          text:
            "Мене часто засмучує те, що я не можу належним чином надати партнерові професійну підтримку, послугу, допомогу.",
        },
        {
          id: 17,
          text:
            "Мені завжди вдається запобігти впливу поганого настрою на ділові контакти.",
        },
        {
          id: 18,
          text:
            "Мене дуже засмучує, якщо щось не ладиться у відносинах з діловим партнером.",
        },
        {
          id: 19,
          text:
            "Я настільки втомлююся на роботі, що вдома намагаюся спілкуватися якнайменше.",
        },
        {
          id: 20,
          text:
            "Через брак часу, втоми або напруги часто приділяю увагу партнеру менше, ніж належить.",
        },
        {
          id: 21,
          text:
            "Іноді самі звичайні ситуації спілкування на роботі викликають роздратування.",
        },
        {
          id: 22,
          text:
            "Я спокійно сприймаю обгрунтовані претензії партнерів.",
        },
        {
          id: 23,
          text:
            "Спілкування з партнерами спонукало мене цуратися людей.",
        },
        {
          id: 24,
          text:
            "При згадці про деяких колег по роботі або партнерів у мене псується настрій.",
        },
        {
          id: 25,
          text:
            "Конфлікти або розбіжності з колегами забирають багато сил і емоцій.",
        },
        {
          id: 26,
          text:
            "Мені все важче встановлювати або підтримувати контакти з діловими партнерами.",
        },
        {
          id: 27,
          text:
            "Обстановка на роботі мені здається дуже важкою, складною.",
        },
        {
          id: 28,
          text:
            "У мене часто виникають тривожні очікування, пов'язані з роботою: щось має статися, як би не допустити помилки, чи зможу зробити все, як треба, не скоротять чи і т. П.",
        },
        {
          id: 29,
          text:
            "Якщо партнер мені неприємний, я намагаюся обмежити час спілкування з ним або менше приділяти йому уваги.",
        },
        {
          id: 30,
          text:
            "У спілкуванні на роботі я дотримуюся принципу: «не роби людям добра, не отримаєш зла».",
        },
        {
          id: 31,
          text:
            "Я охоче розповідаю домашнім про свою роботу.",
        },
        {
          id: 32,
          text:
            "Бувають дні, коли мій емоційний стан погано позначається на результатах роботи (менше роблю, знижується якість, трапляються конфлікти).",
        },
        {
          id: 33,
          text:
            "Часом я відчуваю, що треба виявити до партнера емоційну чуйність, але не можу.",
        },
        {
          id: 34,
          text:
            "Я очень переживаю за свою работу.",
        },
        {
          id: 35,
          text:
            "Партнерам по работе отдаешь внимания и заботы больше, чем получаешь от них признательности.",
        },
        {
          id: 36,
          text:
            "При мысли о работе мне обычно становится не по себе: начинает колоть в области сердца, повышается давление, появляется головная боль.",
        },
        {
          id: 37,
          text:
            "У меня хорошие (вполне удовлетворительные) отношения с непосредственным руководителем.",
        },
        {
          id: 38,
          text:
            "Я часто радуюсь, видя, что моя работа приносит пользу людям.",
        },
        {
          id: 39,
          text:
            "Последнее время (или как всегда) меня преследуют неудачи в работе.",
        },
        {
          id: 40,
          text:
            "Некоторые стороны (факты) моей работы вызывают глубокое разочарование, повергают в уныние.",
        },
        {
          id: 41,
          text:
            "Бывают дни, когда контакты с партнерами складываются хуже, чем обычно.",
        },
        {
          id: 42,
          text:
            "Я разделяю деловых партнеров (субъектов деятельности) на «хороших» и «плохих».",
        },
        {
          id: 43,
          text:
            "Усталость от работы приводит к тому, что я стараюсь сократить общение с друзьями и знакомыми.",
        },
        {
          id: 44,
          text:
            "Я обычно проявляю интерес к личности партнера помимо того, что касается дела.",
        },
        {
          id: 45,
          text:
            "Обычно я прихожу на работу отдохнувшим, со свежими силами, в хорошем настроении.",
        },
        {
          id: 46,
          text:
            "Я иногда ловлю себя на том, что работаю с партнерами автоматически, без души.",
        },
        {
          id: 47,
          text:
            "По работе встречаются настолько неприятные люди, что невольно желаешь им чего-нибудь плохого.",
        },
        {
          id: 48,
          text:
            "После общения с неприятными партнерами у меня бывает ухудшение физического или психического самочувствия.",
        },
        {
          id: 49,
          text:
            "На работе я испытываю постоянные физические или психологические перегрузки.",
        },
        {
          id: 50,
          text:
            "Успехи в работе вдохновляют меня.",
        },
        {
          id: 51,
          text:
            "Ситуация, в которой я оказался на работе, кажется безысходной (почти безысходной).",
        },
        {
          id: 52,
          text:
            "Я потерял покой из-за работы.",
        },
        {
          id: 53,
          text:
            "На протяжении последнего года была жалоба (были жалобы) в мой адрес со стороны партнера(ов).",
        },
        {
          id: 54,
          text:
            "Мне удается беречь нервы благодаря тому, что многое из происходящего с партнерами я не принимаю близко к сердцу.",
        },
        {
          id: 55,
          text:
            "Я часто с работы приношу домой отрицательные эмоции.",
        },
        {
          id: 56,
          text:
            "Я часто работаю через силу.",
        },
        {
          id: 57,
          text:
            "Прежде я был более отзывчивым и внимательным к партнерам, чем теперь.",
        },
        {
          id: 58,
          text:
            "В работе с людьми я руководствуюсь принципом: не трать нервы, береги здоровье.",
        },
        {
          id: 59,
          text:
            "Иногда иду на работу с тяжелым чувством: как все надоело, никого бы не видеть и не слышать.",
        },
        {
          id: 60,
          text:
            "После напряженного рабочего дня я чувствую недомогание.",
        },
        {
          id: 61,
          text:
            "Контингент партнеров, с которым я работаю, очень трудный.",
        },
        {
          id: 62,
          text:
            "Иногда мне кажется, что результаты моей работы не стоят тех усилий, которые я затрачиваю.",
        },
        {
          id: 63,
          text:
            "Если бы мне повезло с работой, я был бы более счастлив.",
        },
        {
          id: 64,
          text:
            "Я в отчаянии из-за того, что на работе у меня серьезные проблемы.",
        },
        {
          id: 65,
          text:
            "Иногда я поступаю со своими партнерами так, как не хотел бы, чтобы поступали со мной.",
        },
        {
          id: 66,
          text:
            "Я осуждаю партнеров, которые рассчитывают на особое снисхождение, внимание.",
        },
        {
          id: 67,
          text:
            "Чаще всего после рабочего дня у меня нет сил заниматься домашними делами.",
        },
        {
          id: 68,
          text:
            "Обычно я тороплю время: скорей бы рабочий день кончился.",
        },
        {
          id: 69,
          text:
            "Состояния, просьбы, потребности партнеров обычно меня искренне волнуют.",
        },
        {
          id: 70,
          text:
            "Работая с людьми, я обычно как бы ставлю экран, защищающий от чужих страданий и отрицательных эмоций.",
        },
        {
          id: 71,
          text:
            "Работа с людьми (партнерами) очень разочаровала меня.",
        },
        {
          id: 72,
          text:
            "Чтобы восстановить силы, я часто принимаю лекарства.",
        },
        {
          id: 73,
          text:
            "Как правило, мой рабочий день проходит спокойно и легко.",
        },
        {
          id: 74,
          text:
            "Мои требования к выполняемой работе выше, чем то, чего я достигаю в силу обстоятельств.",
        },
        {
          id: 75,
          text:
            "Моя карьера сложилась удачно.",
        },
        {
          id: 76,
          text:
            "Я очень нервничаю из-за всего, что связано с работой.",
        },
        {
          id: 77,
          text:
            "Некоторых из своих постоянных партнеров я не хотел бы видеть и слышать.",
        },
        {
          id: 78,
          text:
            "Я одобряю коллег, которые полностью посвящают себя людям (партнерам), забывая о собственных интересах.",
        },
        {
          id: 79,
          text:
            "Моя усталость на работе обычно мало сказывается (никак не сказывается) в общении с домашними и друзьями.",
        },
        {
          id: 80,
          text:
            "Если предоставляется случай, я уделяю партнеру меньше внимания, но так, чтобы он этого не заметил.",
        },
        {
          id: 81,
          text:
            "Меня часто подводят нервы в общении с людьми на работе.",
        },
        {
          id: 82,
          text:
            "Ко всему (почти ко всему), что происходит на работе, я утратил интерес, живое чувство.",
        },
        {
          id: 83,
          text:
            "Работа с людьми плохо повлияла на меня как профессионала — обозлила, сделала нервным, притупила эмоции.",
        },
        {
          id: 84,
          text:
            "Работа с людьми явно подрывает мое здоровье.",
        },
      ],

      criterions: {

        TENSION_CIRCUMSTANCES: "Переживание психотравмирующих обстоятельств",
        TENSION_SELF_SATISFACTION: "Удовлетворенность собой",
        TENSION_CAGED: "«Загнанность в клетку»",
        TENSION_DEPRESSION: "Тревога и депрессия",

        RESISTANCE_EMOTIONAL_RESPONSE: "Неадекватное избирательное эмоциональное реагирование",
        RESISTANCE_DISORIENTATION: "Эмоционально-нравственная дезориентация",
        RESISTANCE_EXPANDING_THE_SCOPE: "Расширение сферы экономии эмоций",
        RESISTANCE_PROF_DUTIES: "Редукция профессиональных обязанностей",

        DEPLETION_EMOTIONAL_DEFECITS: "Эмоциональный дефицит",
        DEPLETION_EMOTIONAL_DETACHMENT: "Эмоциональная отстраненность",
        DEPLETION_DEPERSONALIZATION: "Личностная отстраненность (деперсонализация)",
        DEPLETION_PSYCHO_DISORDERS: "Психосоматические и психовегетативные нарушения",

        TENSION: "напряжение",
        RESISTANCE: "резистенция",
        DEPLETION: "истощение",

        SYMBOL_PLUS: "+",
        SYMBOL_MINUS: "-",

      },

      common: {

        WELCOME_TEXT: `Добро пожаловать в бот социальной поддержки CodeFlexers 
          \nОн поможет вам справиться с проблемами профессионального выгорания. 
          \nПосле короткого теста вы получите информацию о состоянии вашего эмоционального выгорания и рекомендации, как справиться с ситуацией. 
          \n\nНажмите /language, чтобы выбрать язык.\n\nНажмите /questionnaire чтобы начать опрос`,
        WRONG_COMMAND: "Неправильная команда, попробуйте еще раз!",
        CHOOSE_LANGUAGE_TEXT: "Какой язык вы предпочитаете?",
        LANGUAGE_CHANGED: `Язык успешно изменен. 
          \n\nНажмите /language, чтобы выбрать язык. 
          \n\nНажмите /questionnaire чтобы начать опрос`,

      }
    },
  },
};
